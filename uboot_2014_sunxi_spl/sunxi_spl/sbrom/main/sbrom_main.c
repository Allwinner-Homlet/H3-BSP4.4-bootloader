...
static toc_name_addr_tbl toc_name_addr[] = {
	...
	{ITEM_CPUS_CODE_NAME,SUNXI_CPUS_CODE_BUFF_ADDR}
#endif
};
...
{
	pr_force("HELLO! pmu_init stub called!\n");
	return 0;
}
...
#ifndef CONFIG_SUNXI_ARISC_NOT_RESET
...
#endif
#ifdef CONFIG_CPUS_STANDBY
                                else if(!strcmp(item_group.binfile->name, ITEM_CPUS_CODE_NAME))
				{
					*(uint *)(SUNXI_CPUS_CODE_SIZE_ADDR) = item_group.binfile->data_len;
					sunxi_flash_read(item_group.binfile->data_offset/512,
                                                (item_group.binfile->data_len+511)/512,
                                                (void *)SUNXI_CPUS_CODE_BUFF_ADDR);
				}
                                else if(!strcmp(item_group.binfile->name, ITEM_SOCCFG_NAME))
				{
					*(uint *)(SUNXI_SYSCONFIG_SIZE_ADDR) = item_group.binfile->data_len;
					sunxi_flash_read(item_group.binfile->data_offset/512,
                                                (item_group.binfile->data_len+511)/512,
                                                (void *)SUNXI_SYSCONFIG_BUFF_ADDR);
