...
#define OPTEE_SECURE_DRM_MBYTES_DEF 80
unsigned int go_exec (u32 monitor_entry, u32 uboot_entry, int optee_entry, unsigned int dram_size)
{
	...
	{
		pr_notice("toc0_config->secure_without_OS is 0\n");
		sunxi_spc_set_to_ns(0);
		if (toc0_config->secure_dram_mbytes == 0)
		{
			/*unit:megbyte, if not set from sysconfig, use default*/
			pr_force("sysconfig not set,use default secure_dram_mbytes config\n");
			toc0_config->secure_dram_mbytes = OPTEE_SECURE_DRM_MBYTES_DEF;
		}
	...
	{
		pr_notice("!!toc0_config->secure_without_OS is none zero\n");
	}
