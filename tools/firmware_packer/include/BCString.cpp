...
{
	//printf("user operator= CBCString\n");
	if(m_pchData)
		delete m_pchData;
	int nLen = stringSrc.GetLength() + 1;
	m_pchData = new char[nLen];
...
void CBCString::TrimRight()
{
	...
	while(nIndex >= 0)
	{
		if(m_pchData[nIndex] == ' ' ||m_pchData[nIndex] == '\t')
		{
			m_pchData[nIndex] = '\0';
			nIndex--;
			continue;
...
//	printf("strLen  =%d\n", nLen);
	...
	int nIndex = 0;//GetLength() - 1;
	int nNewIndex = 0;
	int nCount = 0;
	while(nIndex < nLen -1)
	{
	//	printf("m_pchData[%d] %c\n", nIndex, m_pchData[nIndex] );
...
{
	//printf("Enter CBCString::Format \n");
	va_list argList;
	va_start(argList, lpszFormat);
	FormatV(lpszFormat, argList);
	va_end(argList);
	//printf("Leave CBCString::Format \n");
}
